-- Add portfolio publish/draft system like LinkedIn
-- This allows editing in private and publishing when ready
...
CREATE TABLE IF NOT EXISTS portfolio_snapshots (
    ...
    -- Indexes
    UNIQUE(org_id, status) WHERE status = 'published'
);
...
COMMENT ON TABLE portfolio_snapshots IS 'Stores published versions of portfolios for public viewing';
COMMENT ON FUNCTION publish_portfolio(VARCHAR) IS 'Publishes entire portfolio as a snapshot for public viewing';
COMMENT ON FUNCTION unpublish_portfolio(VARCHAR) IS 'Unpublishes portfolio, making it draft-only';
COMMENT ON FUNCTION get_published_portfolio(VARCHAR) IS 'Gets the published portfolio data for public viewing';